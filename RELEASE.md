...
> `publish-operator-hub.yaml`). This allows downstream `pull_request` workflows
> (`go.yaml`, `ci.yaml`) to fire on the auto-created PR. If `persesbot` does
> not have write access to this repository, PR creation will fail — in that
> case, fall back to `${{ secrets.GITHUB_TOKEN }}` in `release.yaml`
